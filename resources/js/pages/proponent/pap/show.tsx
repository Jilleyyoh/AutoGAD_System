        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="bg-white dark:bg-slate-800 shadow overflow-hidden sm:rounded-lg">
            ...
            <div className={`px-4 py-3 border-b ${
              ...
              status === 'for_revision' || status === 'revision' ? 'bg-orange-50 dark:bg-orange-900/20 border-orange-200 dark:border-orange-800' :
              'bg-gray-50 dark:bg-gray-900/20 border-gray-200 dark:border-gray-800'
            }`}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-medium ${
                    status === 'pending' ? 'bg-yellow-100 dark:bg-yellow-900/50 text-yellow-800 dark:text-yellow-200' :
                    status === 'approved' ? 'bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-200' :
                    status === 'rejected' ? 'bg-red-100 dark:bg-red-900/50 text-red-800 dark:text-red-200' :
                    status === 'for_revision' || status === 'revision' ? 'bg-orange-100 dark:bg-orange-900/50 text-orange-800 dark:text-orange-200' :
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 px-4 py-5 sm:px-6">
              <dl className="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2">
                <div className="sm:col-span-2">
                  ...
                  <dd className="mt-1 text-sm text-gray-900 dark:text-white">{project.title || '—'}</dd>
                </div>
                <div>
                  <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Domain</dt>
                  <dd className="mt-1 text-sm text-gray-900 dark:text-white">{project.domain_expertise?.domain_name || '—'}</dd>
                </div>
                <div>
                  <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Phase</dt>
                  <dd className="mt-1 text-sm text-gray-900 dark:text-white">{project.phase || '—'}</dd>
                </div>
                <div className="sm:col-span-2">
                  <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Description</dt>
                  <dd className="mt-1 text-sm text-gray-900 dark:text-white whitespace-pre-wrap">{project.description || '—'}</dd>
                </div>
              </dl>
            </div>
            ...
            {/* Documents */}
            <div className="px-4 py-5 sm:px-6">
              <h3 className="text-lg leading-6 font-medium text-gray-900 dark:text-white">Project Documents</h3>
            </div>
